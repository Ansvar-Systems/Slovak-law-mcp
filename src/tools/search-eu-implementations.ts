/**
 * search_eu_implementations — Search EU directives/regulations referenced by Slovak legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_slovak_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as slovak_statute_count
  ...
  if (input.has_slovak_implementation) {
    sql += ' HAVING slovak_statute_count > 0';
  }
