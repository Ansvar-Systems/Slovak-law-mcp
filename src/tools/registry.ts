/**
 * Tool registry for Slovak Law MCP Server.
...
import { getSlovakImplementations, type GetSlovakImplementationsInput } from './get-slovak-implementations.js';
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Slovak statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Slovak statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Slovak legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Slovak statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Slovakia?" ' +
  ...
  },
  {
    ...
    description:
      'Format an Slovak legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Slovak statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that an Slovak statute references or aligns with. ' +
      'As an EU Member State, Slovakia transposes EU directives and implements EU regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Slovak statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_slovak_implementations',
    description:
      'Find all Slovak statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Slovak statutes. ' +
      'Note: Slovakia is an EU Member State and transposes EU directives into national law.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Slovak legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_slovak_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Slovak legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Slovak statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for an Slovak statute or provision. ' +
      ...
      'Note: As an EU Member State, Slovakia is bound by EU law. This checks transposition and compliance status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Slovak statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_slovak_implementations':
          result = await getSlovakImplementations(db, args as unknown as GetSlovakImplementationsInput);
          break;
