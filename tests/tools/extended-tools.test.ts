...
/* ------------------------------------------------------------------ */
/* DB-dependent test suites (skipped in CI without DB)                */
/* ------------------------------------------------------------------ */

describe.skipIf(!DB_EXISTS)('extended-tools (DB-dependent)', () => {
  // Open once for all nested suites
  realDb = DB_EXISTS ? new BetterSqlite3(DB_PATH, { readonly: true, fileMustExist: true }) : (undefined as never);
