#!/usr/bin/env tsx
/**
 * Database builder for Slovak Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Slovak Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Slovak statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'SK');
