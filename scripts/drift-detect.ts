#!/usr/bin/env tsx
/**
 * Drift detection for Slovak Law MCP.
 *
 * Checks if upstream www.slov-lex.sk content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Slovak Law MCP — Drift Detection');
  console.log('=====================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'Slovak-Law-MCP/1.0 drift-detect' },
      });
