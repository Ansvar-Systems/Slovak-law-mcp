/**
 * HTML parser for Slovak legislation from the Sejm ELI API (api.sejm.gov.pl).
 *
 ...
 *
 * Slovak legislation references: Dz.U. YYYY poz. NNNN
 ...
 * to extract provisions from a Slovak statute.
 *
 ...
 *   </div>
 */
export function parseSlovakHtml(html: string, act: ActIndexEntry): ParsedAct {
  ...
  for (let i = 0; i < articleStarts.length; i++) {
    ...
    // Slovak acts use "ilekroć mowa" (whenever mentioned), "rozumie się przez to"
...
/**
 * Extract definitions from Slovak legal text.
 *
 * Slovak definitions typically use patterns like:
