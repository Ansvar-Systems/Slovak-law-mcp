#!/usr/bin/env tsx
/**
 * Slovak Law MCP -- Ingestion Pipeline
 *
 * Fetches Slovak legislation from the Sejm ELI API (api.sejm.gov.pl).
 * The Sejm (Slovak Parliament) provides free public access to all legislation
 ...
 * License: Slovak legislation is public domain under Art. 4 of the Copyright Act
 */
...
import { parseSlovakHtml, KEY_SLOVAK_ACTS, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function fetchAndParseActs(acts: ActIndexEntry[], skipFetch: boolean): Promise<void> {
  console.log(`\nProcessing ${acts.length} Slovak Acts from api.sejm.gov.pl...\n`);
  ...
  for (const act of acts) {
    ...
    try {
      ...
      const parsed = parseSlovakHtml(html, act);
  ...
  console.log(`  License:     Public domain (Art. 4 Slovak Copyright Act)`);
...
async function main(): Promise<void> {
  ...
  console.log('Slovak Law MCP -- Ingestion Pipeline');
  console.log('====================================\n');
  ...
  const acts = limit ? KEY_SLOVAK_ACTS.slice(0, limit) : KEY_SLOVAK_ACTS;
